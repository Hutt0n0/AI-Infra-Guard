...
// TaskCreateRequest represents the request structure for creating a task
// Corresponds to the input when creating a task from the frontend
// Example: {"id":..., "sessionId":..., "task":..., ...}
type TaskCreateRequest struct {
	ID             string                 `json:"id" validate:"required"`        // Message ID (dialog ID generated by frontend) - Required
	SessionID      string                 `json:"sessionId" validate:"required"` // Session ID (task ID) - Required
	Username       string                 `json:"username,omitempty"`            // Username (optional, defaults to public user)
	Task           string                 `json:"taskType" validate:"required"`  // Task type - Required
	Timestamp      int64                  `json:"timestamp" validate:"required"` // Timestamp - Required
	Content        string                 `json:"content" validate:"required"`   // Task content description - Required
	Params         map[string]interface{} `json:"params,omitempty"`              // Task parameters - Optional
	Attachments    []string               `json:"attachments,omitempty"`         // Attachment list - Optional
	CountryIsoCode string                 `json:"countryIsoCode,omitempty"`      // Language identifier (optional)
}
